...
export class ProductsService {
  ...
  addProduct(product: FormData) {
    return this.http.post<{ image: string }>("http://localhost:3006/products", product);
  }
 ...
 public async getProductsSearch(productSearch: string): Promise <void> {
  const observable = this.http.get<>(this.config.getOneProduct + productID)
    return this.http.get<Array<GetProduct>>(`http://localhost:3006/products/${productSearch}`)
  }
}
